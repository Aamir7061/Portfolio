...
const TimelineItem = (items: any) => {
    ...
    return items.map((item: any, index: number) => <Timeline.Item data-aos="fade-up" data-aos-duration="800"
        ...
        <div className="border shadow-[0_0_10px_0_#64FFDA50] hover:-translate-y-2 transition transform duration-300 ease-in-out flex flex-col gap-2 border-primaryColor p-4 rounded-2xl sm-mx:p-2">
            <div className="flex gap-2 justify-center mb-10 mt-10 items-center">
            ...
            <div className="text-white md-mx:px-4 px-20 leading-6 text-justify text-xl md-mx:text-sm xs-mx:text-xs">
                <ul className="list-disc pl-6">
                {item.desc.split('.').map((point:any, idx:any) => (
                    <li key={idx}>{point}</li>
                ))}
                </ul>
            </div>

            <div className="text-lg font-bold px-20 justify py-10 text-primaryColor md-mx:text-base sm-mx:text-sm xs-mx:text-xs flex gap-1"><div className="font-semibold text-white">Skills:</div>
...
const Experience = () => {
    ...
    return <div className="px-16 mx-20 md-mx:px-6 sm-mx:px-2 lg-mx:mx-0 my-10 mt-20 mb-28 font-mono" id="Experience">
